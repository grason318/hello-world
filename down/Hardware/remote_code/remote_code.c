...
{	
	...
	if((Control_Mode & auto_control) != auto_control)			//如果控制模式不等于自动控制，即遥控控制
	{
         ...
         static int count_1=1;	
					...
					{trigger_speed = 150;
					    count_1=1;           }
					if(motor5.actual_speed<5&&motor5.actual_speed>-5)    						//堵转
					{ 
						static int count_=1;
					  count_++;
				   ...
				   fric1_on(1000);
				   fric2_on(1000);
					if(motor5.actual_speed<10&&motor5.actual_speed>-10)    						//堵转
					{ 
						static int count_=1;
					  count_++;
         ...
         static int count_1=1;	
				  ...
				  fric1_on(1000);
				  fric2_on(1000);
					if(motor5.actual_speed<10&&motor5.actual_speed>-10)    						//反转
					{ 
						static int count_=1;
					  count_++;
						trigger_speed =pow(-1,count_)*50;
